 ...
 *
 * A few changes were introduced by Filipe Dias to maintain glEnable
 * attribute as it was prior to loading the obj
 */
...
GLvoid glmDraw(GLMmodel * model, GLuint mode)
{
	...
	glPushAttrib(GL_ENABLE_BIT);	// added by Filipe Dias
	if (mode & GLM_COLOR)
	...
	glPopMatrix();
	glPopAttrib();	// added by Filipe Dias to restore glEnable(); settings.
}
